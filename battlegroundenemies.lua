...
function BGE:UpdateIdentity(row, unit)
    ...
    if classFile then
        ...
        local classAlpha2 = (row._outOfRange and CLASS_ALPHA_OOR) or CLASS_ALPHA_ACTIVE
        ApplyClassAlpha(row, classAlpha2)
    end
    UpdateNameClipToHPFill(row)
    ...
    if row.achievIconTex == nil then
        ...
        local iconTex = row.achievIconTex
        if iconTex and iconTex ~= "" and iconTex ~= "none" then
            row.icon:SetTexture(iconTex)
            row.icon:Show()
        else
    ...
    if row.achievTint and row.icon and row.icon.SetVertexColor then
        row.icon:SetVertexColor(row.achievTint[1] or 1, row.achievTint[2] or 1, row.achievTint[3] or 1, row.achievTint[4] or 1)
    elseif row.icon and row.icon.SetVertexColor then
        row.icon:SetVertexColor(1, 1, 1, 1)
    end

    if row.iconHit and row.achievText and row.achievText ~= "" then
        row.iconHit:SetScript("OnEnter", function(f)
            if GameTooltip and GameTooltip.SetOwner then
                GameTooltip:SetOwner(f, "ANCHOR_RIGHT")
                GameTooltip:ClearLines()
                if row.achievText then
                    GameTooltip:AddLine(row.achievText, 1, 1, 1, true)
                end
                GameTooltip:Show()
            end
        end)
        ...
        row.iconHit:Show()
    elseif row.iconHit then
        row.iconHit:SetScript("OnEnter", nil)
        row.iconHit:SetScript("OnLeave", nil)
        row.iconHit:Hide()
    end
...
function BGE:RetryMissingIdentities()
    ...
    for i = 1, (self.maxPlates or 40) do
        local row = self.rows and self.rows[i] or nil
        if row and not row._preview and row.unit and UnitExists(row.unit) then
            if (not row.name) or (not row.classFile) then
